...
def test_add_and_update_visitor(week2_client, visitor_payload):
    add_response = week2_client.post("/visitors/add", json=visitor_payload)
    ...
    update_response = week2_client.put("/visitors/update", json=updated_payload)
...
def test_add_and_update_employee(week2_client, employee_payload):
    add_response = week2_client.post("/employees/add", json=employee_payload)
    assert add_response.status_code == 200
    ...
    update_response = week2_client.put("/employees/update", json=updated_payload)
    assert update_response.status_code == 200
...
def test_capture_visit_and_reports(week2_client, visitor_payload, employee_payload, visit_payload):
    visitor_response = week2_client.post("/visitors/add", json=visitor_payload)
    assert visitor_response.status_code == 200

    employee_response = week2_client.post("/employees/add", json=employee_payload)
    assert employee_response.status_code == 200

    visit_response = week2_client.post("/visits", json=visit_payload)
    assert visit_response.status_code == 200
    assert visit_response.json() == "Visit recorded"

    employee_report = week2_client.get("/reports/employee/Anita%20Sharma")
    ...
    date_report = week2_client.get(f"/reports/date/{visit_payload['visitDate']}")
